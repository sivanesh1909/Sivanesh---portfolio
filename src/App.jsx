...
function App() {
  ...
  return <div className="site-shell">
    ...
    <header className={`nav ${active !== 'home' ? 'nav-scrolled' : ''}`}><button className="wordmark" onClick={() => go('home')}>SIVANESH <span>R</span></button><div className="nav-links">{navItems.map((item) => <button className={active === item ? 'active' : ''} key={item} onClick={() => go(item)}>{item}</button>)}</div><a className="resume-download" href="/Sivanesh2_up.pdf" download="Sivanesh-R-Resume.pdf"><Download size={14} /> RESUME</a><button className="menu-trigger" onClick={() => setMenuOpen(true)} aria-label="Open menu"><Menu size={19} /></button></header>
